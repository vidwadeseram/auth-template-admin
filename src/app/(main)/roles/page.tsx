...
export default function RolesPage() {
  ...
  async function loadRoles() {
    setLoading(true);
    try {
      const res = await apiClient.get("/api/v1/admin/roles") as { data: Role[] };
      setRoles(res.data || []);
    } catch (err: unknown) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {roles.length === 0 ? (
          <Card className="col-span-full"><CardContent className="p-8 text-center text-muted-foreground">No roles loaded. Click &quot;Load Roles&quot; to fetch.</CardContent></Card>
        ) : roles.map((role) => (
          <Card key={role.id}>
            <CardHeader>
